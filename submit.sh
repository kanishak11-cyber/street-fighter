...
if [[ "$OSTYPE" == "msys" || "$OSTYPE" == "win32" ]]; then
    # Windows command prompt doesn't support tput
    bold=""
    normal=""
    green=""
    red=""
...
# Create a flag file to track installation status
INSTALL_FLAG="$PROJECT_DIR/.setup_complete"
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    echo "${red}${bold}Unsupported OS detected. This script supports Windows, macOS, Linux, and WSL.${normal}"
    exit 1
fi

echo "${yellow}Detected OS: $OS_TYPE${normal}"

# Step 1: Install Python 3.9 and dependencies - only if not already done
if [ ! -f "$INSTALL_FLAG" ]; then
    ...
    if [[ "$OS_TYPE" == "macOS" ]]; then
        ...
        if ! command -v python &> /dev/null || ! python --version 2>&1 | grep -q "Python 3.9"; then
            echo "${yellow}Python 3.9 not found. Please install Python 3.9 from https://www.python.org/downloads/release/python-3913/${normal}"
            ...
            # Check again
            if ! command -v python &> /dev/null || ! python --version 2>&1 | grep -q "Python 3.9"; then
                echo "${red}${bold}ERROR: Python 3.9 installation not detected. Please install Python 3.9 and add it to PATH.${normal}"
                exit 1
            fi
        else
            echo "${green}Python 3.9 is installed: $(python --version)${normal}"
        fi
        
        # Check for Docker Desktop
        if ! command -v docker &> /dev/null; then
            echo "${yellow}Docker Desktop not found. Please install Docker Desktop from https://www.docker.com/products/docker-desktop/${normal}"
            echo "Once installed and running, press Enter to continue..."
            read -p ""
            
            # Check again
            if ! command -v docker &> /dev/null; then
                echo "${red}${bold}ERROR: Docker installation not detected. Please install Docker Desktop and add it to PATH.${normal}"
                exit 1
            fi
        else
            echo "${green}Docker is installed: $(docker --version)${normal}"
    ...
    # Create a flag file to prevent reinstallation on next run
    touch "$INSTALL_FLAG"
    echo "${green}${bold}Installation phase completed and won't be repeated on next run.${normal}"
else
    echo "${green}Dependencies already installed. Skipping installation phase.${normal}"
    
    # Set PYTHON_CMD based on OS
...
# Step 2: Create and activate virtual environment (only if it doesn't exist)
echo
echo "${bold}STEP 2: Setting up Python 3.9 virtual environment${normal}"
echo "==================================================="
...
DEPS_FLAG="$PROJECT_DIR/.deps_installed"
...
if [ ! -f "$DEPS_FLAG" ]; then
    ...
    # Create flag file to mark dependencies as installed
    touch "$DEPS_FLAG"
else
    echo "${green}Python dependencies already installed. Skipping installation.${normal}"
fi
...
# Determine how to call diambra based on OS
if [[ "$OS_TYPE" == "Windows" ]]; then
    if [ -f "$VIRTUAL_ENV/Scripts/diambra.exe" ]; then
        DIAMBRA_CLI="$VIRTUAL_ENV/Scripts/diambra.exe"
    else
        DIAMBRA_CLI="diambra"
    fi
else
...
if [ ! -f "$PROJECT_DIR/roms/sfiii3n.zip" ]; then
    ...
    if [[ "$OS_TYPE" == "WSL" ]]; then
        ...
        echo "Please place the ROM file in: $PROJECT_DIR\\roms\\"
    fi
...
# Create custom agent.py template to ensure correct game ID
echo "Creating custom agent.py template..."
cat > "$PROJECT_DIR/output/models/agent_template.py" << 'EOF'
#!/usr/bin/env python3
import os
...
def main():
    model = PPO.load(os.path.join(os.path.dirname(os.path.abspath(__file__)), "1022"))

    # Environment creation
    env = diambra.arena.make("sfiii3n")

    # Disable interaction request
    env.settings.interact_request = False
    ...
    # Environment shutdown (it will never get here due to the while True loop)
    env.close()
...
# Function to fix agent.py after initialization
fix_agent_py() {
    if [ -f "$PROJECT_DIR/output/models/agent.py" ]; then
        echo "Fixing agent.py to use correct game ID..."
        # Fix the environment name in agent.py
        if [[ "$OS_TYPE" == "Windows" ]]; then
            # For Windows, replace the file content
            python -c "
content = open('$PROJECT_DIR/output/models/agent.py', 'r').read()
fixed_content = content.replace('env = diambra.arena.make(\"doapp\")', 'env = diambra.arena.make(\"sfiii3n\")')
open('$PROJECT_DIR/output/models/agent.py', 'w').write(fixed_content)
"
        else
            # Use sed for Unix systems
            sed -i 's/env = diambra\.arena\.make("doapp")/env = diambra\.arena\.make("sfiii3n")/g' "$PROJECT_DIR/output/models/agent.py"
        fi
        echo "${green}Fixed agent.py to use correct game ID${normal}"
    else
        echo "${yellow}agent.py not found. It will be created during the agent initialization process.${normal}"
    fi
}
...
echo "6. Reset Diambra credentials"
echo "7. Exit"
echo
...
case $ACTION_CHOICE in
    ...
    4)
        ...
        if [ ! -f "$PROJECT_DIR/results/sfiii3n/sr6_128x4_das_nc/model/1022.zip" ]; then
            ...
            # Fix agent.py after initialization
            fix_agent_py
    ...
    6)
        echo
        echo "${bold}Resetting Diambra credentials...${normal}"
        
        # Log out first using the CLI
        $DIAMBRA_CLI user logout
        
        # Remove credentials file
        if [[ "$OS_TYPE" == "Windows" ]]; then
            # Windows path is different
            CRED_PATH="$USERPROFILE/.diambra/credentials"
            if [ -f "$CRED_PATH" ]; then
                rm -f "$CRED_PATH"
                echo "Removed credentials file at: $CRED_PATH"
            else
                echo "No credentials file found at: $CRED_PATH"
            fi
            
            # Also check AppData location
            APPDATA_CRED_PATH="$APPDATA/diambra/credentials"
            if [ -f "$APPDATA_CRED_PATH" ]; then
                rm -f "$APPDATA_CRED_PATH"
                echo "Removed credentials file at: $APPDATA_CRED_PATH"
            fi
        else
            # Unix-based system (Linux, macOS, WSL)
            CRED_PATH="$HOME/.diambra/credentials"
            if [ -f "$CRED_PATH" ]; then
                rm -f "$CRED_PATH"
                echo "Removed credentials file at: $CRED_PATH"
            else
        ...
        # Check if Docker is running and restart the engine container if it exists
        if command -v docker &> /dev/null; then
            if docker ps -q --filter "name=engine" | grep -q .; then
                echo "Stopping Diambra engine container..."
                docker stop engine
                
                echo "Starting new Diambra engine container..."
                if [[ "$OS_TYPE" == "Windows" ]]; then
                    # Windows path conversion
                    ROMS_PATH=$(echo "$PROJECT_DIR/roms" | sed 's/\\/\//g')
                else
                    ROMS_PATH="$PROJECT_DIR/roms"
                fi
                
                docker run -d --rm --name engine \
                  -v "$HOME/.diambra/credentials:/tmp/.diambra/credentials" \
                  -v "$ROMS_PATH:/opt/diambraArena/roms" \
                  -p 127.0.0.1:50051:50051 \
                  docker.io/diambra/engine:latest
                
                echo "Diambra engine container restarted."
            fi
        fi
        
        # Force a new login
        echo "${bold}Please log in to your Diambra account:${normal}"
        $DIAMBRA_CLI user login
        
        echo "${green}Credentials reset complete.${normal}"
        ;;
    7)
        echo "Exiting..."
        exit 0
